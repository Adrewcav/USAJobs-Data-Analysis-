...
Bachelor's, Master's, Phd, or N/A, are the values used for the data point **Minimum Degree Level**.
The other data points, **Python**, **R**, **SQL**, and **Tableau** have the binary variables, TRUE or FALSE. 

**Note**, Step 6 presents a challenge, since data such as minimum degree level or key skills for a position are not listed in binary terms or have a defined value. This data is present in 1-2 columns with a 1-300 word description per row item. Therefore, before we can set a single value for each column and row, we will need to perform a separate analysis i.e. keyword scanning via Text Mining. This will allow for the assignment of values based on the presence of a key term in the description.

### Step 4: Text Mining in R
I needed to familiarize myself with text mining, so I picked up a copy of the book "Text Mining in Practice" by Ted Kwartler to learn the basics of Keyword scanning. 
